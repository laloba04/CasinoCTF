  ...
  return (
    <div className="container" style={{ paddingTop: '1rem', paddingBottom: '2rem' }}>
      <div className="page-header">
        <div>
          <h1 className="page-title">🔓 {t('ctfTitle')}</h1>
          <p className="page-subtitle">{t('ctfSubtitle')} • {solvedCount}/9 {t('solved')}</p>
        </div>
        <div style={{
          background: 'var(--accent-purple-dim)', padding: '0.5rem 1.2rem',
          borderRadius: 'var(--radius-xl)', color: 'var(--accent-purple)', fontWeight: 700
        }}>
          {solvedCount}/9 {t('flags')} 🚩
        </div>
      </div>
        ...
        <div className="card fade-in">
          ...
          <div style={{ marginBottom: '1.5rem' }}>
            <div className="form-group">
              <label className="form-label">{t('submitFlag')}</label>
              <div className="flex gap-1">
                <input className="input" placeholder="BJCTF{...}" value={flag}
                  onChange={e => setFlag(e.target.value)} />
                <button className="btn btn-primary" onClick={submitFlag}>{t('submit')} 🚩</button>
              </div>
            </div>
            {message && (
          ...
          <div>
            <button className="btn btn-outline btn-sm" onClick={() => loadHints(selectedChallenge.id)}
              ...
              💡 {hintLevel >= 3 ? t('noMoreHints') : `${t('getHint')} ${hintLevel + 1}/3`}
            </button>
            {hints.length > 0 && (
